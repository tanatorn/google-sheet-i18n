...
try {
  config = require(_path2.default.join(cwd(), 'i18n.config'));
} catch (err) {
  config = null;
}
...
var getSheetRows = function getSheetRows(worksheet) {
  return new _bluebird2.default(function (resolve, reject) {
    return getRows(worksheet).then(function (rows) {
      var newRows = rows.map(formatRow).filter(function (row) {
        return row !== null;
      });
      resolve({
...
var beingTranslations = function beingTranslations(sheets) {
...
var start = function start() {
  if (config) {
    (function () {
      var doc = _bluebird2.default.promisifyAll(new _googleSpreadsheet2.default(sheetId));
      var creds = require(credentialsPath);
      doc.useServiceAccountAuthAsync(creds).then(function () {
        return getInfo(doc);
      }).then(getSheets).then(beingTranslations).then(function () {
